...
public class AdxInRangeThenIncreasing extends Connection{
	public void getHighDeliveryPercDates( java.sql.Connection con, String name){
		...
		try{
			...
			for(int i=0; i< tradedate.size()-1; i++){
				count=0;
				if((float)adx.get(i) >20 && (float)adx.get(i-1) <20){
					for(int j=i-20; j< i; j++){
						if((float)adx.get(j) <20){
							count++;
						}else count =0;
					}
				}
				if(count>15){
					if((float)di_plus.get(i) > (float)di_minus.get(i)){
...
//					if((float)di_plus.get(i) < (float)di_minus.get(i)){
//						trig=(float)open.get(i+1); 
//						percProfitAtHighPrice = (trig-(float)low.get(i+5))*100/trig;
//						percProfitAtClosePrice = (trig-(float)close.get(i+5))*100/trig;
//						sql = "insert into williamsresults(name, reversal, triggerPrice, profitPerc, profitRupees, date) "
...
//					}
   ...
   public static void main(String[] args)  {
	      Test t = new Test();
	      AdxInRangeThenIncreasing pin = new AdxInRangeThenIncreasing();
