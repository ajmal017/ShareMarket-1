...
public class OBV_Increased extends Connection{
			...
			for(int i=0; i< tradedate.size()-1; i++){
				...
				if(count> 200){
					for(int j=i; j< tradedate.size()-1; j++){
						if((long)obv.get(j) < 0 && (float)close.get(j)>(float)open.get(j)){
							percProfitAtClosePrice = ((float)close.get(j+50) - (float)open.get(j))*100/(float)open.get(j);
							sql = "insert into williamsresults(name, reversal, triggerPrice, profitPerc, profitRupees, date, dateexited) "
				  					+ " values ('"+name+"', 'Bull', "+percProfitAtClosePrice+", "+percProfitAtClosePrice+", "
				  							+ ""+percProfitAtHighPrice+", '"+tradedate.get(j)+"', '"+tradedate.get(j+50)+"')";
   ...
   public static void main(String[] args)  {
	      Test t = new Test();
	      OBV_Increased pin = new OBV_Increased();
