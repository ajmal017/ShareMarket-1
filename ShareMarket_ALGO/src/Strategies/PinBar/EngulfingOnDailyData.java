...
public class EngulfingOnDailyData extends Connection{
		...
		try{
			...
			for(int i=0; i< tradedate.size()-1; i++){
				...
				if(prev1 >1 && prev2>1 && prev3>1 && prev4>1 && (float)open.get(i) < (float)close.get(i-1) && (float)close.get(i) > (float)open.get(i-1) && 
						(float)close.get(i)>(float)open.get(i) && width>1 && (float)high.get(i+1)>(float)high.get(i) && (float)open.get(i) < (float) s2.get(i) ){
   ...
   public static void main(String[] args)  {
	      Test t = new Test();
	      EngulfingOnDailyData pin = new EngulfingOnDailyData();
