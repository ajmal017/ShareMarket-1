...
public class Gap_PrevDayUpperLowerShadowAvailableWithCheapPrice extends Connection{
	public void getHighDeliveryPercDates( java.sql.Connection con, String name){
		...
		List perc = new ArrayList<Float>();
	  	...
	  	Gap_PrevDayUpperLowerShadowAvailableWithCheapPrice gap = new Gap_PrevDayUpperLowerShadowAvailableWithCheapPrice();
		String date="",sql="";
		float percProfitAtHighPrice=0f, percProfitAtClosePrice=0f, percProfitAtLowPrice=0f;
		try{
//			name= name+"_FUT";
			sql = "select * from  "+name+" as a where a.tradedate > '2010-01-01'";
//			sql = sql + " and avg_volume*lotsize*open > 2000000000";
			...
			float trig=0f, prev1,prev2,prev3,prev4, width;float gapPerc=2f,gapLimitPerc=5f, div=2f;int iter=1;String out="";
			float stopLossPerc=1f, targetPerc=1f;
			for(int i=1; i< tradedate.size(); i++){
				float diff =0f, stopLoss=0f;
				if(((float)open.get(i-1)-(float)close.get(i-1))*100/(float)open.get(i-1) > gapPerc && 
						((float)high.get(i-1)-(float)open.get(i-1))*100/(float)high.get(i-1) > gapPerc*2
					)
				{
					float trigger = ((float)open.get(i) - (float)open.get(i)*0.2f/100);
					if(((float)open.get(i)-(float)close.get(i-1))*100/(float)close.get(i-1) > gapPerc)
					{
						...
						stopLoss =  ((float)high.get(i) - trigger)*100/(float)trigger;
						String isFaded="0";
						sql = "insert into williamsresults(name, reversal, triggerPrice, profitPerc, profitRupees, date) "
			  					+ " values ('"+name+"', 'Bear', "+percProfitAtLowPrice+", "+percProfitAtClosePrice+", "+(float)open.get(i)+", '"+tradedate.get(i)+"')";
						executeSqlQuery(con, sql);
					}
				}
				if(((float)close.get(i-1)-(float)open.get(i-1))*100/(float)open.get(i-1) > gapPerc && 
						((float)open.get(i-1)-(float)low.get(i-1))*100/(float)low.get(i-1) > gapPerc*2
						)
				{
					float trigger = ((float)open.get(i) + (float)open.get(i)*0.2f/100);
					if(((float)close.get(i-1)-(float)open.get(i))*100/(float)open.get(i) > gapPerc)
					{
						...
						String isFaded="0";
						sql = "insert into williamsresults(name, reversal, triggerPrice, profitPerc, profitRupees, date) "
			  					+ " values ('"+name+"', 'Bull', "+percProfitAtHighPrice+", "+percProfitAtClosePrice+", "+(float)open.get(i)+", '"+tradedate.get(i)+"')";
						executeSqlQuery(con, sql);
					}
				}
			}
   ...
   public static void main(String[] args)  {
	      Test t = new Test();
	      Gap_PrevDayUpperLowerShadowAvailableWithCheapPrice pin = new Gap_PrevDayUpperLowerShadowAvailableWithCheapPrice();
	      ...
	      try{
	    	  ...
	    	  Connection con = new Connection();
	    	  	...
	    	  	ResultSet rs=null;
	    	  	String sql="";
	    	  	sql =  "SELECT s.name FROM symbols s where volume > 100000000  and lastPrice > 5 and lastPrice < 5000 order by volume desc";
//	    	  	sql =  "SELECT s.name FROM symbols s, margintables m where s.name=m.name and lastPrice >20";
