...
public class NoTradeDay extends Connection{
	public void getHighDeliveryPercDates( java.sql.Connection con, String name){
		...
		try{
			...
			for(int i=0; i< tradedate.size()-1; i++){
				if((float)open.get(i)==(float)close.get(i) && (float)high.get(i)==(float)low.get(i)
						&& (float)open.get(i-1)!=(float)close.get(i-1)){
					percProfitAtClosePrice = ((float)close.get(i+2)-(float)open.get(i+1))*100/(float)open.get(i+1);
