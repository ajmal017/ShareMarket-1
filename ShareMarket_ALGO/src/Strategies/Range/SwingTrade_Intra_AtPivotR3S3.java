...
public class SwingTrade_Intra_AtPivotR3S3 extends Connection{

	   public static void main(String[] args)  {
		      Test t = new Test();
		      SwingTrade_Intra_AtPivotR3S3 range = new SwingTrade_Intra_AtPivotR3S3();
			  		...
			  		for (int i=1; i< date.size()-1; i++){
			  			if(rangeHigh==0 || rangeLow==0) break;
							...
							if((float)low.get(i) < triggerPrice && (float)low.get(i-1) > triggerPrice){
			  					sql = "select max(high) from "+name+"_"+interval+" where tradedate <='"+date.get(i)+"' and tradedate >= concat(Date('"+date.get(i)+"'),' 09:10:00')";
		          				...
		          				if((Float.parseFloat(dayshigh) - (float)low.get(i))*100/(float)low.get(i) < 10)
		          					break;
		          				sql = "select max(high) from "+name+"_"+interval+" where tradedate >'"+date.get(i)+"' and tradedate <= concat(Date('"+date.get(i)+"'),' 15:10:00')";
		          				dayshigh = executeCountQuery(con, sql);
...
//			  				if((float)low.get(i) < triggerPrice && (float)low.get(i-1) > triggerPrice 
//			  						&& (float)macd.get(i) < (float)signal.get(i)){
//			  					
...
//		          				sql = "select close from "+name+"_5 where tradedate = concat(Date('"+date.get(i)+"'),' 15:15:00')";
//		          				daysclose = executeCountQuery(con, sql);
//		          				sql = "select close from "+name+"_"+interval+" where tradedate >'"+date.get(i)+"' and tradedate <= concat(Date('"+date.get(i)+"'),' 15:15:00')"
//		          						+ " and macd>sig order by tradedate limit 1";
//		          				exitPrice = executeCountQuery(con, sql);
...
//		          				}
//		          				if(exitPrice.equals("")) break;
//		          				profitPerc = (triggerPrice-Float.parseFloat(exitPrice))*100/triggerPrice;
