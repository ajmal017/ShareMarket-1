...
public class RangeCrossWithMacd extends Connection{

	   public static void main(String[] args)  {
		      Test t = new Test();
		      RangeCrossWithMacd range = new RangeCrossWithMacd();
			  		...
			  		for (int i=2; i< date.size()-1; i++){
			  			if(rangeHigh==0 || rangeLow==0) break;
							...
							if((float)high.get(i) > triggerPrice && (float)high.get(i-1) < triggerPrice 
		          				...
		          				profitPerc = (Float.parseFloat(exitPrice) - triggerPrice)*100/triggerPrice;
