...
public class RangeCrossModified extends Connection{

	   public static void main(String[] args)  {
		      Test t = new Test();
		      RangeCrossModified range = new RangeCrossModified();
	   ...
	   public void getdate(java.sql.Connection con, String name) throws SQLException{
		   ...
		   boolean aboveYestHigh, belowYestLow;
		   for(int i=0; i<date.size()-1; i++){
			   aboveYestHigh = ((float)open.get(i+1) > (float)high.get(i));
			   belowYestLow = ((float)open.get(i+1) < (float)low.get(i));
			   UpdatePinResultsRange(con, name, date.get(i+1).toString(), aboveYestHigh, belowYestLow);
		  	}
		   rs.close();
	   }
	   
	   public void UpdatePinResultsRange(java.sql.Connection con, String name,String dailyDate, boolean aboveYestHigh, boolean belowYestLow) throws SQLException {
		   ResultSet rs=null;
				 ...
				 daysOpen = executeCountQuery(con, "select open from "+name+" where tradedate=Date('"+dailyDate+"')");
			  		for (int i=2; i< date.size()-1; i++){
			  			if(rangeHigh==0 || rangeLow==0) break;
			  				...
			  				triggerPrice = (rangeHigh + .10f);
			  				Float targetProfit = 2f;
							if((float)high.get(i) > triggerPrice && (float)high.get(i-1) < triggerPrice && (belowYestLow==true || aboveYestHigh==true)){
			  				...
			  				triggerPrice = (rangeLow - .10f);
			  				if((float)low.get(i) < triggerPrice && (float)low.get(i-1) > triggerPrice && (belowYestLow==true || aboveYestHigh==true) ){
