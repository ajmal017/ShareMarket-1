...
public class Price_Open_And_High_LowSame extends Connection{
	public void getData( java.sql.Connection con, String name){
		...
		try{
			...
			for(int i=10; i< tradedate.size()-1; i++){
				count=0;
				if((float)open.get(i) == (float)low.get(i)){
					for(int j=i-10; j< i ; j++){
						if((float)close.get(j) < (float)open.get(j)){
							count++;
						}
					}
					if(count>7 && (float)close.get(i) > (float)ema2.get(i) && (float)open.get(i) < (float)ema2.get(i) 
							&& (float)close.get(i-1) < (float)open.get(i-1)){
						
						sql = "select close from "+name+" where tradedate > '"+tradedate.get(i)+"' AND TRADEDATE < DATE_ADD('"+tradedate.get(i)+"', INTERVAL 50 DAY)"
								+ " order by TRADEDATE desc limit 1 ;";
//						float nextMaxHigh = Float.parseFloat(executeCountQuery(con, sql));
   ...
   public static void main(String[] args)  {
	      Test t = new Test();
	      Price_Open_And_High_LowSame pin = new Price_Open_And_High_LowSame();
