...
function gapWithOpenLowOpenHighSameWithGoodVolume(name, zerodhaId, openPrice, yestOpen, yestHigh, yestLow, yestClose, min, max,gapUpPerc,gapDownPerc, res, yestVolume)
{
	var strategyType = 'gapWithOpenLowOpenHighSameWithGoodVolume';
	if(res && res.data && res.data.candles.length > 0){
		o = res.data.candles[0][1];
		h = res.data.candles[0][2];
		l = res.data.candles[0][3];
		v = res.data.candles[0][5];
...
function getEligibleSymbols(){
	...
	for(var i=start; i< end; i++){
		...
		pastAvgVolume = parseFloat(symbols[i][11]);
		if(openPrice==0){
			continue;
		}
		var intraData = getIntradayFromZerodha(zerodhaId);
		...
		gapWithOpenLowOpenHighSameWithGoodVolume(name, zerodhaId, openPrice, yestOpen, yestHigh, yestLow, yestClose, min, max,gapUpPerc,gapDownPerc, intraData, pastAvgVolume);	
		gapCheckFirst3MinBodyWithGoodVolume(name, zerodhaId, openPrice, yestOpen, yestHigh, yestLow, yestClose, min, max,gapUpPerc,gapDownPerc, intraData, pastAvgVolume);
