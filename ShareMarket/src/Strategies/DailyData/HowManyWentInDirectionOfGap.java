...
public class HowManyWentInDirectionOfGap extends Connection{
	public void getHighDeliveryPercDates( java.sql.Connection con, String name){
		...
		List perc = new ArrayList<Float>();
	  	...
	  	HowManyWentInDirectionOfGap gap = new HowManyWentInDirectionOfGap();
		String date="",sql="";
		float percProfitAtHighPrice=0f, percProfitAtClosePrice=0f, percProfitAtLowPrice=0f;
		try{
			rs = executeSelectSqlQuery(con, "select * from  "+name+" as a where a.tradedate>='2016-01-01'");
			while(rs.next()){
			...
			float trig=0f, prev1,prev2,prev3,prev4, width;float gapPerc=1f,gapLimitPerc=8f, div=10f;int iter=1;String out="";
			float stopLossPerc=1f, targetPerc=.2f;
			for(int i=1; i< tradedate.size(); i++){
				...
				if(((float)open.get(i)-(float)high.get(i-1))*100/(float)high.get(i-1) > gapPerc  && ((float)open.get(i)-(float)high.get(i-1))*100/(float)high.get(i-1) < gapLimitPerc 
						&& ((float)open.get(i)-(float)low.get(i))*100/(float)open.get(i) > 0.2){
					sql = "insert into williamsresults(name, reversal, triggerPrice, profitPerc, profitRupees, date) "
		  					+ " values ('"+name+"', 'Bear', 0, "+percProfitAtLowPrice+", "+diff+", '"+tradedate.get(i)+"')";
				...
				if(((float)low.get(i-1)-(float)open.get(i))*100/(float)open.get(i) > gapPerc && ((float)low.get(i-1)-(float)open.get(i))*100/(float)open.get(i) < gapLimitPerc
						&& ((float)high.get(i)-(float)open.get(i))*100/(float)open.get(i)>0.2){
					sql = "insert into williamsresults(name, reversal, triggerPrice, profitPerc, profitRupees, date) "
		  					+ " values ('"+name+"', 'Bull', 0, "+percProfitAtHighPrice+", "+diff+", '"+tradedate.get(i)+"')";
					executeSqlQuery(con, sql);
				}
			}
		}catch(Exception e){
   ...
   public static void main(String[] args)  {
	      Test t = new Test();
	      HowManyWentInDirectionOfGap pin = new HowManyWentInDirectionOfGap();
