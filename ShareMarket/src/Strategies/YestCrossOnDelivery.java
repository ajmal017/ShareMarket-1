...
public class YestCrossOnDelivery extends Connection{
		  	...
		  	try {
				rs = executeSelectSqlQuery(con, "select * from "+name+"_dp a, "+name+" b where  a.tradedate=b.tradedate;");
			  	while (rs.next()){
			  		...
			  		date.add(rs.getString("a.tradedate"));
					perc.add(rs.getFloat("perc"));
					tradedQuantity.add(rs.getLong("tradedQuantity"));
			  	}
			  	for(int x=0; x<date.size()-1; x++){
			  		if((float)open.get(x+1) < (float)high.get(x) && (float)open.get(x+1) > (float)low.get(x) && (long)tradedQuantity.get(x)>(long)tradedQuantity.get(x-1)*2)
			  			calculateYestCross(con, name, (float)perc.get(x), (float)high.get(x), (float)low.get(x), (float)high.get(x+1), (float)low.get(x+1), (float)close.get(x+1), date.get(x+1).toString());
			  	}
			  	if(rs!=null) rs.close();
		  	}
	...
	public void calculateYestCross(java.sql.Connection con, String  name, float prevPerc, float prevHigh, float prevLow,float nextHigh,float nextLow,float nextClose, String nextDate) throws SQLException{
		ResultSet rs=null;
	  	List open = new ArrayList<Float>();
	  	List high = new ArrayList<Float>();
	  	List low = new ArrayList<Float>();
   ...
   public static void main(String[] args)  {
	      Test t = new Test();
	      YestCrossOnDelivery pin = new YestCrossOnDelivery();
