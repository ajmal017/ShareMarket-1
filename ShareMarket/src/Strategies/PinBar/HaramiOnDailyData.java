...
public class HaramiOnDailyData extends Connection{
	public void getHighDeliveryPercDates( java.sql.Connection con, String name){
		...
		try{
			...
			for(int i=0; i< tradedate.size()-1; i++){
				...
				if(prev1 >1 && prev2>1 && prev3>1 && (float)open.get(i) > (float)close.get(i-1) && (float)close.get(i) < (float)open.get(i-1) && 
						(float)close.get(i)>(float)open.get(i) && width>1 && (float)close.get(i-1) <(float)close.get(i-2) && (float)close.get(i-2) <(float)close.get(i-3)
						&& (float)close.get(i+1) > (float)open.get(i+1)){
					trig=(float)open.get(i+2); 
					
					percProfitAtHighPrice = ((float)high.get(i+2)-trig)*100/trig;
					percProfitAtClosePrice = ((float)close.get(i+2)-trig)*100/trig;
					sql = "insert into williamsresults(name, reversal, triggerPrice, profitPerc, profitRupees, date) "
		  					+ " values ('"+name+"', 'Bull', "+percProfitAtClosePrice+", "+percProfitAtClosePrice+", "+percProfitAtHighPrice+", '"+tradedate.get(i+2)+"')";
				...
				if(prev1 >1 && prev2>1 && prev3>1  && (float)open.get(i) < (float)close.get(i-1) && (float)close.get(i) > (float)open.get(i-1) && 
					...
					percProfitAtLowPrice = (trig - (float)low.get(i+2))*100/trig;
					percProfitAtClosePrice = (trig-(float)close.get(i+2))*100/trig;
					sql = "insert into williamsresults(name, reversal, triggerPrice, profitPerc, profitRupees, date) "
		  					+ " values ('"+name+"', 'Bear', "+percProfitAtClosePrice+", "+percProfitAtClosePrice+", "+percProfitAtLowPrice+", '"+tradedate.get(i+2)+"')";
					executeSqlQuery(con, sql);
				}
			}
		}catch(Exception e){
   ...
   public static void main(String[] args)  {
	      Test t = new Test();
	      HaramiOnDailyData pin = new HaramiOnDailyData();
