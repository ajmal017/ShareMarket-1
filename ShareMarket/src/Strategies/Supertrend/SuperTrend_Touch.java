...
public class SuperTrend_Touch extends Connection{
	public void getData( java.sql.Connection con, String name){
		...
		float percentage=80f;
	  	...
	  	List up = new ArrayList<Float>();List down = new ArrayList<Float>();List supertrend = new ArrayList<Float>();
		String date="",sql="";
		...
		try{
			...
			while(rs.next()){
				...
				open.add(rs.getFloat("open"));
		  		...
		  		supertrend.add(rs.getFloat("supertrend"));
			}
			float trig=0f;String tradedDate="", exitDate="";
			int count = 0;
			float o, h,l,c,s;
			for(int i=1; i< tradedate.size(); i++){
				o = (float)open.get(i);h = (float)high.get(i);l = (float)low.get(i);c = (float)close.get(i);
				s = (float)supertrend.get(i-1);
				float diff = (h-c)*100/c;
				if( h > s && c< s && diff >2){
...
//					percProfitAtClosePrice = ((float)high.get(i+1) - (float)open.get(i+1))*100/(float)open.get(i+1);
					sql = "insert into williamsresults(name, reversal, triggerPrice, profitPerc, profitRupees, date, dateexited) "
		  					+ " values ('"+name+"', 'Bull', "+percProfitAtClosePrice+", "+open.get(i)+", "
		  							+ ""+percProfitAtClosePrice+", '"+tradedate.get(i)+"', '"+tradedate.get(i)+"')";
					executeSqlQuery(con, sql);
				}
			}
   ...
   public static void main(String[] args)  {
	      Test t = new Test();
	      SuperTrend_Touch pin = new SuperTrend_Touch();
