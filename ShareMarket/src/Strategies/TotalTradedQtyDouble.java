...
public class TotalTradedQtyDouble extends Connection{

	   public static void main(String[] args)  {
		      Test t = new Test();
		      TotalTradedQtyDouble range = new TotalTradedQtyDouble();
		      ...
		      try{
		    	  ...
		    	  Connection con = new Connection();
		    	  	...
		    	  	ResultSet rs=null;
		    	  	rs = con.executeSelectSqlQuery(dbConnection, "SELECT s.name FROM symbols s where volume > 10000000 and s.name<>'Mindtree'  order by volume desc ");
		    	  	String name="";
	   ...
	   public void getdate(java.sql.Connection con, String name){
		...
		" when cci_reversal='Bear' then 'Bear' when envlp_cross='Bear' then 'Bear' end),'') as Bear,open,high,low,close,max_past_high, min_past_low, s1, s2 "
		+ " from "+name+" b, "+name+"_dp dp where b.tradedate=dp.tradedate";
		   ...
		   try {
			rs = executeSelectSqlQuery(con, sql);
		  	...
		  	List perc = new ArrayList<Float>();List max_high = new ArrayList<Float>();List min_low = new ArrayList<Float>();
		  	List s1 = new ArrayList<Float>();List s2 = new ArrayList<Float>();
		   ...
		   float profitPerc, profitRupees;
		   for(int i=5; i<dailyDate.size()-1; i++){
			   if( (long)tradedQuantity.get(i)>(long)tradedQuantity.get(i-1)*2  && ((float)close.get(i-5)-(float)close.get(i))*100/(float)close.get(i) >10
					   && (float)open.get(i+1) < (float)s1.get(i+1)){
				   profitPerc = ((float)close.get(i+1) - (float)open.get(i+1))*100/(float)open.get(i+1);
				   profitRupees = ((float)high.get(i+1) - (float)open.get(i+1))*100/(float)open.get(i+1);
 					sql = "insert into williamsresults (name, reversal, triggerPrice, profitPerc, profitRupees, date, dateexited, noofcandles) "
        					+ " values ('"+name+"', 'Bull', "+profitPerc+", "+(profitPerc-0.05f)+", "+(profitRupees-0.05f)+", '"+dailyDate.get(i)+"', '"+dailyDate.get(i)+"', '1')";
	   ...
	   public void UpdatePinResultsRange(java.sql.Connection con, String name,String dailyDate, String bull, String bear, List open, List high, List low, List close, List date, List volume) throws SQLException {
		   ResultSet rs=null;
		   List pivot = new ArrayList<Float>();List cci = new ArrayList<Float>();
...
//			  				 rangeHigh > (float)pivot.get(i) && rangeHigh < (float)r1.get(i)
							if((float)cci.get(i) > 100 && (float)cci.get(i-1) < 100  && bull.equals("Bull")){
...
//			  				&& dailyRange >2 && (int)volume.get(i)> (int)volume.get(i-1)*2
			  				...
			  				if((float)cci.get(i) < -100 && (float)cci.get(i-1) > -100  && bear.equals("Bear")){
